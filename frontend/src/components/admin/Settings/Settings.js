...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (userEmail) {
        try {
          const response = await axios.get(`http://localhost:5000/api/admin/settings/${userEmail}`);
          if (response && response.data) {
            const { name, email } = response.data;
            setName(name);
            setEmail(email);
          } else {
            console.error('Failed to fetch user data: No data received');
          }
        } catch (error) {
  ...
  const handleModalSubmit = async (e) => {
    ...
    if (currentField === 'name') {
      ...
      try {
        const response = await axios.put('http://localhost:5000/api/admin/settings', {
          currentField,
          newData,
          userEmail
        });
